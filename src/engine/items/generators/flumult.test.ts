...
function tier(n: number): number {
  if ([0, 1, 2, 5].includes(n)) return 0
...
describe('FLU.MULT multiplication facts', () => {
  ...
  it('bands by how hard the fact is, not by how big the numbers are', () => {
    ...
    const hardestAt = (d: number) => Math.max(...itemsAt(d).map((i) => hardness(i.params.a!, i.params.b!)))
    const easiestAt = (d: number) => Math.min(...itemsAt(d).map((i) => hardness(i.params.a!, i.params.b!)))

    expect(hardestAt(EASIEST)).toBe(0)
